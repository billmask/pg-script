...
#Temp filename
FILENAME=/tmp/vacuum_full_erp_tables.txt
...
vacuum_relation() {
	...
	cat $FILENAME | awk '{print $1 " " "'${statement_timeout}'" " " "'${db}'"}' | xargs -P $JOBS -l bash -c 'echo "psql --set=statement_timeout=$1 --set=ON_ERROR_STOP=ON $2 -AXqtc \"vacuum full analyze $0;\" >&2 "; psql --set=statement_timeout=$1 --set=ON_ERROR_STOP=ON $2 -AXqtc "vacuum full analyze $0;" >&2; sleep 5;'
...
for db in $( psql -AXtqc "SELECT datname FROM pg_database WHERE datname IN ('erpteste') ORDER BY 1;"); do
	#Creates vacuum matrix for this DB
        log "Processing database ${db}"
        log "Analyzing table bloat for database ${db}"
	psql -AXtqf coleta_fragmentacao_tabelas.sql "${db}"
